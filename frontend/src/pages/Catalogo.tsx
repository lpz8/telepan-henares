...
export default function Catalogo() {
  ...
  const handleFoto = (file: File) => {
    ...
    reader.onload = e => { const b64 = e.target?.result as string; setFotoPreview(b64); fSet('foto_base64', b64) }
  ...
  const save = async () => {
    ...
    if (editing) {
      ...
      globalToast('Articulo actualizado')
    } else {
      await supabase.from('catalogo').insert({ ...form, user_id: user.id })
      globalToast('Articulo anadido')
    }
  ...
  const deleteItem = async (id: string) => {
    ...
    globalToast('Articulo eliminado'); load()
  }
  ...
  const buildHTML = () => {
    ...
    const cardsHTML = porCat.map(cat => {
      ...
      const itemsHTML = catItems.map(item => [
        ...
        item.foto_base64 ? '<img src="' + item.foto_base64 + '" alt="' + item.nombre + '"/>' : '<span style="font-size:2.5rem">pan</span>',
        '</div>',
        '<div class="card-body">',
        '<div class="card-nombre">' + item.nombre + '</div>',
        item.descripcion ? '<div class="card-desc">' + item.descripcion + '</div>' : '',
        '<div class="card-precio">' + (item.precio_label || (item.precio > 0 ? Number(item.precio).toFixed(2) + ' EUR' : 'Consultar')) + '</div>',
        '</div></div>'
      ].join(''))
      return [
        '<div class="seccion">',
    ...
    return '<!DOCTYPE html><html><head><meta charset="UTF-8">' +
      ...
      '</style></head><body>' +
      '<div class="portada"><h1>' + EMISOR.nombre + '</h1>' +
      '<div class="slogan">' + EMISOR.slogan + '</div>' +
      '<div class="tel">Tel: ' + EMISOR.telefono + '</div></div>' +
      cardsHTML +
      '<div class="footer">' + EMISOR.nombre + ' - ' + EMISOR.telefono + ' - ' + new Date().getFullYear() + '</div>' +
      '</body></html>'
  }
    ...
    <div>
      ...
      <div className="tabs" style={{ marginBottom: 16, flexWrap: 'wrap' }}>
        {['Todas', ...CATEGORIAS].map(cat => (
          <div key={cat} className={'tab ' + (catFiltro === cat ? 'active' : '')} onClick={() => setCatFiltro(cat)}>{cat}</div>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="card"><div className="empty-state">
          ...
          <span>Pulsa Anadir para empezar</span>
        </div></div>
      ) : (
            ...
            <div key={item.id} className="card" style={{ padding: 0, opacity: item.activo ? 1 : 0.55 }}>
              <div style={{ height: 180, overflow: 'hidden', borderRadius: '10px 10px 0 0', background: '#f5f0eb', cursor: 'pointer', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 8 }}
                ...
                {item.foto_base64
                  ...
                  : <><span style={{ fontSize: '3rem' }}>foto</span><span style={{ fontSize: '0.75rem', color: 'var(--gris)' }}>Pulsa para anadir foto</span></>
                }
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setOpenModal(false)}>
          <div className="modal" style={{ maxWidth: 520 }}>
            <div className="modal-header">
              <h3 className="modal-title">{editing ? 'Editar articulo' : 'Nuevo articulo'}</h3>
            ...
            <div className="modal-body">
              <div style={{ marginBottom: 14 }}>
                ...
                <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                  <div style={{ width: 100, height: 100, borderRadius: 10, overflow: 'hidden', border: '2px dashed #f5e8d8', background: '#fdf8f3', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                    ...
                    {fotoPreview ? <img src={fotoPreview} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontSize: '2rem' }}>foto</span>}
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    <button className="btn btn-secondary btn-sm" onClick={() => document.getElementById('foto-modal')?.click()}>{fotoPreview ? 'Cambiar foto' : 'Anadir foto'}</button>
              ...
              <div className="input-group">
                ...
                <input className="input" value={form.nombre} onChange={e => fSet('nombre', e.target.value)} placeholder="Nombre del articulo..." />
              </div>
              <div className="input-group">
                <label className="input-label">Descripcion</label>
                <input className="input" value={form.descripcion} onChange={e => fSet('descripcion', e.target.value)} placeholder="Descripcion breve..." />
              </div>
              <div className="form-grid-2">
                ...
                <div className="input-group">
                  <label className="input-label">Precio (EUR)</label>
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-primary" onClick={save}>{editing ? 'Guardar cambios' : 'Anadir'}</button>
            </div>
          </div>
        </div>
      )}
