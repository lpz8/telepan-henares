      ...
      {tab === 'lista' && (
        <div className="card" style={{ padding: 0 }}>
          <div className="table-wrap">
            <table>
              <thead>
                <tr><th style={{width:100}}>Acciones</th><th>Cód.</th><th>Nombre</th><th>Zona</th><th>Pago</th><th>Teléfono</th><th>Deuda</th></tr>
              </thead>
              <tbody>
                {filtered.map(c => (
                  <tr key={c.id}>
                    <td>
                      <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                        <button className="btn btn-primary btn-sm btn-icon" onClick={() => openEdit(c)} title="Editar"><Edit2 size={14} /></button>
                        <button className="btn btn-danger btn-sm btn-icon" onClick={() => handleDelete(c.id)} title="Eliminar"><Trash2 size={14} /></button>
                        <button className="btn btn-secondary btn-sm btn-icon" title="Historial" onClick={() => verHistorial(c)}>📋</button>
                        <button
                          title={c.telefono1 ? 'WhatsApp' : 'Sin teléfono'}
                          style={{ background: c.telefono1 ? '#25D366' : '#ccc', color: 'white', border: 'none', fontWeight: 800, fontSize: '0.72rem', padding: '4px 6px', borderRadius: 6, cursor: c.telefono1 ? 'pointer' : 'not-allowed' }}
                          onClick={() => {
                            if (!c.telefono1) return
                            const tel = c.telefono1.replace(/\D/g, '')
                            const msg = deudas[c.id] > 0
                              ? encodeURIComponent(`Hola ${c.nombre}, le recordamos que tiene una deuda pendiente de ${deudas[c.id].toFixed(2)}€. Gracias, TelePan Henares 🍞`)
                      ...
                      {c.forma_pago === 'Domiciliación' && cuentas[c.id] && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 2 }}>
                          ...
                          {cuentasVisibles.has(c.id) ? (
                            <span style={{ fontSize: '0.72rem', fontFamily: 'monospace', color: '#92400e', fontWeight: 700 }}>{cuentas[c.id]}</span>
                          ) : (
                            <span style={{ fontSize: '0.72rem', color: '#ca8a04', fontWeight: 700 }}>IBAN: ••••••••••</span>
                          )}
                    ...
                    <td>
                      {deudas[c.id] > 0
                        ? <span style={{ color: '#dc2626', fontWeight: 800, fontSize: '0.82rem' }}>💰 {deudas[c.id].toFixed(2)} €</span>
                        : <span style={{ color: '#16a34a', fontSize: '0.8rem' }}>✅</span>}
                    </td>
                  </tr>
                ))}
                {filtered.length === 0 && (
                  <tr><td colSpan={7}><div className="empty-state"><p>No hay clientes</p></div></td></tr>
                )}
              </tbody>
            </table>
          </div>
